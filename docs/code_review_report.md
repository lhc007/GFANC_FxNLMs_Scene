...
### 7.2 当前必须修复（Phase-1，本周，均不依赖硬件）

| 序 | 项 | 工作量 |
|----|----|--------|
| 1 | ~~**R-1** K 残留硬编码 8（活跃堆溢出）~~ ✅ 已修复 (2026-07-27, `523b692`) | 0.5 天 |
| 2 | ~~**R-3** 加载校验 + **R-4** K 交叉校验~~ ✅ 已修复 (2026-07-27, `6dddf85`) | 0.5 天 |
| 3 | ~~**R-2** 离线末块越界~~ ✅ 已修复 (2026-07-27, `6dddf85`) | 0.2 天 |
| 4 | ~~**R-8** 输入 isfinite + FIR 中毒看门狗~~ ✅ 已修复 (2026-07-27) | 0.5 天 |
| 5 | **R-6** 静音期梯度冻结 + **R-7** freeze Wc 回滚 | 0.5 天 |
...
| 时点 | 项 |
|------|----|
| Phase-2（树莓派）前 | R-21 osal/HAL 抽象 + anc_process_block 提取；R-19 stdatomic 映射；R-11/R-12 性能（1×5×4 前提）；R-29 通道数集中化；R-5 帧长假设（若用非 ASIO 路径） |
| Phase-3（MCU/DSP）前 | R-23 延迟线 float32 化；R-22 VLA 消除；R-24/R-25 静态化与 CNN 瘦身；R-27 权重内嵌+manifest；硬件看门狗（量产，历史 P2-4） |
| 硬件窗口期 | **R-13** bp 降阶/双通路（NR 带宽最大杠杆，需硬件 A/B）；**R-16** Ŝ 实测校准（F-B，git revert 162d357）；R-14/R-15 抽取/内插滤波 |

### 7.4 建议路线

```
本周(纯软件):  R-1 → R-3/R-4 → R-2 → R-6/R-7/R-8 → R-9/R-10/R-26 → R-31基线
     （全部可由离线 main.exe + ASan 验证, 零硬件依赖）
硬件可用时:    R-16 Ŝ实测 → R-13 bp降阶 A/B → R-14/R-15
Phase-2 启动:  R-21/R-19 → R-11/R-12 → R-29 (1×5×4) → RPi 部署 (PREEMPT_RT 或加大周期)
Phase-3 评估:  芯片选型 (§5.3 表) → R-23/R-22/R-24/R-25 → 定点化仿真 (QCC 路线才需要)
```
