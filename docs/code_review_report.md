...
- **修复状态**：✅ **已修复** (2026-07-29, `72164c8`) — 实现 free_conv/free_bn/free_resblock 辅助函数，释放 stem+4×resblock+fc 全部权重指针 (~600KB) + 推理激活缓冲 g_cnn_buf (~1MB)。g_cnn_buf 提升为文件作用域。支持 init→free→init 热循环。
...
### H. 硬件实测发现（2026-07-28 实时运行暴露）

> 以下 5 项为连接 UMC 404HD + ASIO 实际运行后暴露的问题，纯代码审查无法发现。
> 均在本次会话中修复。

#### R-36 calibrate_feedback 输入通道硬编码 6 + 设备列表过滤排除 ASIO · **一般 · [Phase-1]**

- **位置**：calibrate_feedback.c:170（`PaStreamParams` 输入通道 `6`）、:150（`defaultSampleRate >= 48000`）
- **问题描述**：① 输入通道硬编码为 6，但 UMC 404HD 只有 4 路输入 → `Pa_OpenStream` 失败 "Invalid number of channels"。`main_realtime.c` 使用 4 通道（1 ref + 3 err），校准程序应与之匹配。② 设备列表过滤 `defaultSampleRate >= 48000` 将 ASIO 设备（默认 44100Hz）全部排除 → 用户看不到 ASIO 设备号，无法选中。
- **造成的影响**：校准程序对 4 输入通道声卡不可用；ASIO 用户在设备列表中找不到目标设备。
- **修复方案**：① 输入通道 6→4，与 main_realtime.c 一致（`in[i*4 + 0]`）；② 设备列表改为 main_realtime.c 同款按 API 分组显示（跳过 MME/DirectSound，显示 ASIO/WASAPI/WDM-KS），移除 `>=48000` 过滤。
- **修复状态**：✅ **已修复** (2026-07-28) — 两处修改，校准程序对 UMC ASIO Driver (in=4 out=4 fs=44100) 正常工作。

#### R-37 Ŝ (secondary_path) 导出尺度非物理 + Ŝ 归一化与 step_size 耦合 · **严重 · [Phase-1]**
...
- **位置**：gfanc_types.h:54（`mic_pre_gain` 默认 10.0f）
- **问题描述**：反馈抵消虽然设计了 `fb_fir[2]`，但 `data/feedback_path_{0,1}.bin` 需通过 `calibrate_feedback.exe` 手动生成——首次用户必然缺失这两个文件。此时 `fb_active=0`，扬声器→参考麦的声学耦合未经抵消，gain=10x (+20dB) 直接将反噪声放大 10 倍喂回参考通道 → 正反馈振荡（啸叫）。UMC 404HD 实测反馈路径 FIR peak≈0.0001（-80dB 耦合，声学隔离良好），但 gain=10x 仍足以触发。
- **造成的影响**：首次用户体验 = 开机立即啸叫 → Wc 发散 → 永久静音，系统看起来"完全不可用"。降 gain 到 3x 后啸叫消失（配合反馈抵消启用）。
- **修复方案**：默认 gain 10.0f→3.0f（+9.5dB），用户可通过 `GFANC_MIC_GAIN` 环境变量按需调高。校准后反馈抵消启用时可安全恢复高增益。
- **验证方法**：无 `feedback_path_*.bin` 时以 gain=3 启动 → 无啸叫，反馈抵消启用后 FB est > 0；逐步调高 gain 到 10x 验证稳定性。

#### R-40 README 快速开始缺失反馈校准步骤 — 首次用户跳过校准直达实时运行 · **建议 · [Phase-1]**

- **位置**：README.md §快速开始（编译→运行，无校准步骤）
- **问题描述**：README 原有"反馈路径校准"独立章节但不在快速开始流程中，用户按编号步骤 1→2→3→4 走完编译直接运行实时版，必然遇到 R-39 的啸叫问题。校准程序本身的编译命令也未在快速开始中出现。
- **造成的影响**：首次用户必然踩坑；校准程序另有 R-36 的设备枚举 bug → 双重阻塞。
- **修复方案**：快速开始增加"步骤 4：校准反馈路径"（编译+运行说明），原独立章节精简为引用。校准程序编译命令、设备选择、安静环境要求均在快速开始中明确。
- **修复状态**：✅ **已修复** (2026-07-28) — README 重构，校准步骤纳入快速开始流程。

#### R-47 x_hist 前向通路时间反转 — anti = Wc ⊗ x_hist 的 Wc 索引与梯度训练方向相反 · **致命 · [Phase-1]**

- **位置**：fxnlms_mimo.c:141-156（`fxnlms_forward_rt`）、:169-185（`fxnlms_tick_rt`）
- **问题描述**：`x_hist_push` 写 `x_hist[ptr]` 后 ptr 指向下一个写入位（即最旧样本）。旧代码以 `hp = x_hist_ptr` 为起点向 0 递减读取，k=0 对齐的是 **最旧** 样本。而梯度更新使用 `seg1 = (p==0)?L-1:p-1`（xd 最新样本位置），k=0 正确对齐 **最新** 样本。结果：Wc 在梯度方向被训练为 `wc[0]=响应最新输入`，但前向通路应用为 `wc[0]=响应最旧输入`——**时间完全反转**。这是 R-12 环形缓冲改造（commit `2853ba0`）引入的索引错误。
- **造成的影响**：反噪声输出相位完全错误；解释了日志中 antiEst（正确对齐）与 anti_rms（反转对齐）之间 100-400× 的巨大差距；场景记忆 scene_wc 保存的 Wc 正确训练但被反转应用。
- **修复方案**：前向通路改为从 `newest = (hp==0) ? L-1 : hp-1` 开始读取，k=0 对齐最新样本。
- **修复状态**：✅ **已修复** (2026-07-28)

#### R-48 NLMS 功率 floor 位置错误 — 有效 floor 被 E·L 除后缩小 3072× · **严重 · [Phase-1]**

- **位置**：fxnlms_mimo.c:104-116（`fxnlms_tick` 离线）、:191-203（`fxnlms_tick_rt` 实时）
- **问题描述**：`power[s] = (1e-6 + ΣXd²) / (E·L)`，ε=1e-6 被 E·L=3072 除后有效 floor 仅 ~3.26×10⁻¹⁰。step_size=5×10⁻⁷，最大有效步长 ≈1534。安静信号（ref_filt RMS≈0.0003）下 power≈2×10⁻¹⁰，持续以 μ_eff≈1500 更新 → Wc 慢性膨胀 → antiEst 飙至 0.19-0.43 → 诚实 NR 虚高至 48-52dB。
- **修复方案**：`power[s] = ΣXd²/(E·L) + 1e-6f`，floor 移到除法后，最大步长 = 0.5。离线+实时双路径同步修复。
- **修复状态**：✅ **已修复** (2026-07-28)

#### R-49 antiEst/anti_rms 比值异常检测 · **一般 · [Phase-1]**

- **位置**：main_realtime.c:499-504
- **问题描述**：antiEst/anti_rms >20× 时 Wc 膨胀或 Ŝ 失配，NR 读数虚高。
- **修复方案**：`print_diagnostics` 增加比值检查与告警打印。
- **修复状态**：✅ **已修复** (2026-07-28)

#### R-50 反馈路径 spk1 校准极弱 · **建议 · [Phase-1]**
...
- **位置**：main_realtime.c:496-508
- **问题描述**：日志显示 ref RMS 仅 0.0002-0.004（-74~-48dBFS），ANC 环路增益严重不足，受限于 ADC 量化噪声地板。用户需要知道当前电平和建议增益值。
- **修复方案**：`print_diagnostics` 新增电平诊断——ref RMS < 0.005 时自动计算并打印建议的 `GFANC_MIC_GAIN` 值（目标 ref_rms≈0.03 = -30dBFS）；ref RMS > 0.3 时警告过载。
- **修复状态**：✅ **已修复** (2026-07-28)

#### P3 啸叫检测阈值提高 · **一般 · [Phase-1]**

- **位置**：include/howling_detect.h:21-22
- **问题描述**：125Hz 环境窄带（房间驻波/变压器）持续在 10-12dB 触发旧阈值边界。真正声学反馈啸叫峰均值比通常 >20dB。
...
#### R-58 关键参数未经系统性标定 — wc_rms_target 和 leak 为工程经验值 · **一般 · [Phase-1]**

- **位置**：main_realtime.c:851（`wc_target = 0.03f`）、gfanc_types.h:61（`leak = 5e-6f`）、scene_controller.c:196-198（Wc RMS 缩放）
- **问题描述**：两个对收敛速度和稳定性有直接影响的参数未经网格搜索或理论推导确定：
  - **wc_rms_target = 0.03**：由 `stub_rms(0.0124) × 2.4` 得到。2.4× 是试出来的折中——大了初始 anti 太强会触发 peak_mute，小了 LMS 收敛太慢。但没有系统性地扫过 [1.0×, 2.0×, 3.0×, 5.0×, 10.0×] 看最优值在哪。
  - **leak = 5e-6**（每秒衰减 ~7.7%）：在 1e-6（太慢，安静期 Wc 漂移）和 1e-4（太快，把有用 Wc 泄掉）之间取了个中间值。没有在 [1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5] 上做过收敛速度 vs 安静期稳定性的 trade-off 分析。
- **造成的影响**：当前值"能跑"，但不一定是最优。wc_rms_target 偏小 → 初始收敛慢 → 前几秒 NR 低；偏大 → 初始饱和 → peak_mute → Wc 被削半。leak 偏小 → 安静期 Wc 漂移积累；偏大 → 有用 Wc 被泄掉 → 噪声再来时需重新收敛。
- **修复方案**：离线版（main.c）做网格搜索，因为离线可以批量跑、NR_true 提供精确评估：
  ```bash
  # wc_rms_target 网格搜索
  for wc_scale in 0.5 1.0 2.0 3.0 5.0 10.0; do
      GFANC_WC_TARGET=$(echo "0.0124 * $wc_scale" | bc) ./main.exe road_noise-15.wav
      # 记录 avg NR_true 和前 5 秒 NR_true 收敛曲线
  done

  # leak 网格搜索
  for leak in 1e-6 2e-6 5e-6 1e-5 2e-5 5e-5; do
      GFANC_LEAK=$leak ./main.exe road_noise-15.wav
      # 记录 avg NR_true 和安静段 Wc RMS 变化
  done
  ```
  取 NR_true 最高且不触发异常的 wc_rms_target 和 leak 作为最优值。离线标定结果直接适用于实时版（参数语义一致）。
  另外需要在 gfanc_types.h 增加 `GFANC_WC_TARGET` 环境变量覆盖入口（当前 wc_rms_target 只能在 main_realtime.c 中硬编码修改）。
- **验证方法**：网格搜索结果中，最优 wc_rms_target 应使 NR_true 前 5 秒平均最高（收敛最快），最优 leak 应使安静段（ref_rms < 0.001）的 Wc RMS 变化 <5%/s。两者在不同噪声类型（road_noise、mixed_7types）上应有一致的最优值。
- **修复状态**：📋 **待标定** (2026-07-29)
...
| 代码健壮性 | 7 | R-1→R-4 全部修复；硬件实测发现并修复 R-36→R-40（校准/增益/Ŝ） |
| 可移植性 | 5 | 算法核心干净，但 Win32 直绑/VLA/double/CNN 4MB 四道 Phase-3 门槛 |
| 可扩展性 | 6 | K 已动态化（但有 R-1 残留）；E/S/L 仍六点硬编码 |
| 可测试性 | 5 | 黄金回归+FIR/FxNLMS单测就位；校准程序经验证可用；实时 CI 仍需硬件 |
| 文档 | 8 | 快速开始含校准步骤；R-32 漂移项已同步 |
...
| 6 | **R-5** WASAPI 变长回调 `fcount%3≠0` — 跨回调相位保持 | 严重 | Phase-1 | `fb64bdf` |
| 7 | **R-10** UAF + **R-9** cfg 死字段/env 无效 + **R-26** PaHostApiInfo2 ABI | 一般 | Phase-1 | `16ec532` |
| 8 | **R-14** 48k→16k 抽取抗混叠 (biquad fc=6.5kHz) + **R-15** ZOH→线性内插抗镜像 | 一般 | Phase-1 | `83ef232` |
| 9 | **R-17** 啸叫检测三通道平均→取 max (避免单通道稀释 ~5dB) | 一般 | Phase-1 | `fb64bdf` |
| 10 | **R-31** 黄金回归测试基线 + FIR/FxNLMS 单测 + `make test` | 一般 | Phase-1 | `3c30f05` |
| 11 | **R-32** README 文档漂移 (K=8→4, ASIO→多后端, 24-bit→16-bit) | 建议 | Phase-1 | `c6c1570` |
| 12 | **R-34** export_bin.py (fade_len + GFANC_PYTHON_PROJ) + **R-35** ResBlock proj 前向兼容 | 建议 | Phase-1 | `545b058` |
| 13 | **R-18** 离线工具链: 16-bit 校验 + 重采样抗混叠 + dB(Full) 瞬态消除 | 一般 | Phase-1 | `2acb9db` |
| 14 | **R-11** anti_est 降采样 250/秒 (省~16%预算) + **R-12** xd/x_hist 环形缓冲 (消 7168 拷贝/样本) | 一般 | Phase-1 | `2853ba0` |
| 15 | **R-20** CNN 双缓冲丢帧检测 + 日志告警 | 建议 | Phase-1 | `fb64bdf` |
| 16 | **R-36** calibrate_feedback 输入通道硬编码 6→4 + 设备列表排除 ASIO (fs=44100) | 一般 | Phase-1 | 2026-07-28 |
| 17 | **R-37+R-38** Ŝ 物理尺度 (移除归一化, s_cal, 固定 step_size=5e-7) | 严重 | Phase-1 | `2d41e78` |
| 19 | **R-39** 默认 mic_pre_gain 10x→3x (反馈抵消缺失时防啸叫) | 一般 | Phase-1 | 2026-07-28 |
| 20 | **R-40** README 快速开始加入反馈校准步骤 | 建议 | Phase-1 | 2026-07-28 |
| 21 | **R-41** peak_mute 硬门控 ↔ 发散 Wc 极限环 ("嘟嘟嘟" 根因) — 上升沿 Wc×0.5 + 释放迟滞 160 样本 + out_gain 平滑包络 + 静音期 Wc 持续衰减 (半衰期 0.25s) | 严重 | Phase-1 | 2026-07-28 |
| 22 | **R-42** NR 公式 (pe+pa)/pe 发散态反向指示 (实测 err 恶化 20× 时显示 36dB) — 诚实 NR=d=err−anti_est + DIV 标志; 连带修复 scene_wc 被发散快照污染 (假 NR 通过收敛判定) | 严重 | Phase-1 | 2026-07-28 |
| 23 | **R-43** max\|Wc\| 发散判据过宽 (anti_rms=0.63 持续失控不触发) — anti_rms 连续 3s > diverge_anti_rms(0.25) → Wc 救援回滚 + ramp | 严重 | Phase-1 | 2026-07-28 |
| 24 | **R-44** 啸叫阈值 15dB 高于限幅梳状谱实测峰 (11-13dB, 全程 notches=0) — err 阈值 12dB + 新增 anti 通道检测 10dB (振荡必经输出, 峰不被扰动稀释) | 一般 | Phase-1 | 2026-07-28 |
| 25 | **R-45** 场景单帧即切泵浦 + scene_wc_valid 置位不写 Wc (恢复到全零滤波器) — 3 帧滞回 + CNN 预设同步存入场景记忆 | 一般 | Phase-1 | 2026-07-28 |
| 26 | **R-46** calibrate 抽取无抗混叠 (与 R-14 链路不一致) + 弱路径无警告; 默认参数 gain 3→1 / step 1e-6→5e-7 / leak 1e-6→5e-6 | 一般 | Phase-1 | 2026-07-28 |
| 27 | **R-47** x_hist 前向通路时间反转 — anti=Wc⊗x_hist 的 Wc 索引与梯度训练方向相反 (wc[0]对齐最旧而非最新样本) | 致命 | Phase-1 | `207a8c2` |
| 28 | **R-48** NLMS 功率 floor 位置错误 — 有效 floor 被 E·L 除后缩小 3072×, 安静信号有效步长爆炸~1534 | 严重 | Phase-1 | `207a8c2` |
| 29 | **R-49** antiEst/anti_rms 比值异常检测 — 比值>20× 时 NR 可能虚高, 新增诊断告警 | 一般 | Phase-1 | `207a8c2` |
| 30 | **P1** 输入电平 SPL 诊断 — ref RMS 过低/过高时自动建议 GFANC_MIC_GAIN 调节值 | 一般 | Phase-1 | `207a8c2` |
| 31 | **P3** 啸叫检测阈值提高 — err 12→14dB, anti 10→12dB, 滤除 125Hz 环境窄带伪峰 | 一般 | Phase-1 | `207a8c2` |
| 32 | **R-52** peak_mute 屏蔽 Wc 发散检测 — wc_snapshot 在 Wc×0.5 后捕获, freeze 永不触发 | 严重 | Phase-1 | `79b9afc` |
| 33 | **R-53** check_wc_divergence 重复 anti_rms 救援代码块 (L596-609 删除) | 一般 | Phase-1 | `79b9afc` |
| 34 | **R-54** diverge_anti_rms 自适应 mic_pre_gain — gain=5-8× 下有效阈值不再过高 | 一般 | Phase-1 | `79b9afc` |
| 35 | **R-55** anti_est 250 样本窗口 LCG 伪随机化 — 消除与 1s 边界对齐的系统性偏差 | 一般 | Phase-1 | `79b9afc` |
...
| 序 | 项 | 级别 | 工作量 | 阻碍 |
|----|----|------|--------|------|
| 41 | **R-16-②** .bin 文件加 16B 格式头 `{magic,version,dims,crc32}` + bin_load_float 校验 | 严重 | 0.5 天 | 需 export_bin.py 同步改 + 全部 .bin 重新导出 |
| 42 | **R-13** bp FIR 1024→256tap 降阶 (群延迟 32→8ms) — ANC 通路用 256tap, CNN 保留 1024tap | 严重 | 1 天 | 需 Python 训练侧生成新 bandpass_256.bin；A/B 验证需硬件 |

#### 🟡 硬件窗口期（需声卡/扬声器/麦克风实测）

| 序 | 项 | 级别 | 工作量 | 阻碍 |
|----|----|------|--------|------|
| 17 | **R-16-①** Ŝ 次级路径实测校准 — `git revert 162d357` 恢复 calibrate_secondary.c, ASIO 实测 E×S=6 条 Ŝ | 严重 | 1 天 | 需 ASIO 声卡 + 扬声器 + 麦克风闭环 |
| 18 | **R-16-③** Ŝ 校准质量门禁 — FIR 峰值位置/RMS 超差拒绝写入 | 一般 | 0.2 天 | 依赖 R-16-① 实测工具就位 |
| 43 | **R-50** 反馈路径 spk1 校准验证 — 当前 RMS≈2×10⁻⁵, R-47 修复后 anti 增大可能暴露反馈耦合 | 建议 | 0.2 天 | 需重新运行 calibrate_feedback.exe 并确认 |
| 44 | **R-51** 125Hz 窄带声源排查 — 关闭 ANC 录制频谱, 确认 125Hz 是房间驻波/变压器/ANC 反馈 | 建议 | 0.3 天 | 需硬件录制 + 频谱分析 |

#### 🟠 Phase-2 前必须重构（树莓派 ARM Linux）

| 序 | 项 | 级别 | 工作量 |
|----|----|------|--------|
| 19 | **R-21** 无音频 I/O HAL — Win32 API 贯穿主程序 (Sleep/Interlocked*/CtrlHandler) | 严重 | 1 天 |
| 20 | **R-29** 通道数 E/S/L 硬编码分布 6 文件 — 1×5×4 扩展需多点同步 | 严重 | 0.5 天 |
| 21 | **R-19** Interlocked* → stdatomic 映射 (x86 闭环成立, ARM 需 C11 atomic) | 一般 | 0.3 天 |
| 22 | **R-30** 梯度有效步长 ∝ E — E 变更必须重调 μ | 一般 | 0.2 天 |
| 23 | **R-24** 动态内存需静态化 — 慢速路径 4 处 malloc 改静态池 | 一般 | 0.3 天 |
| 24 | **R-27** 权重 .bin 无格式头/manifest — 版本混配无防线 (注: 格式头实现在 R-16-②, 此项指 manifest.json 与 sha256) | 一般 | 0.5 天 |

#### 🔵 Phase-3 前必须重构（MCU/DSP 裸机）

| 序 | 项 | 级别 | 工作量 |
|----|----|------|--------|
| 25 | **R-23** FIR 延迟线 double — Cortex-M 软浮点 10-30× 减速 + RAM 翻倍 | 严重 | 0.5 天 |
| 26 | **R-22** 变长数组 VLA — MSVC/IAR/Keil 不可编译 | 严重 | 0.2 天 |
| 27 | **R-25** CNN 激活缓冲 4MB — Phase-3 RAM 最大单项, 需分块/复用降至 <512KB | 一般 | 1 天 |
| 28 | **R-28** 其他平台依赖: getenv/printf/time()/`##__VA_ARGS__`/大小端等 | 一般 | 0.5 天 |

### 7.3 架构优化机会（待评估，均需 Python 训练侧或硬件配合）

> 以下 5 项为 2026-07-29 架构审查识别，经逐一验证**均非纯 C 可独立完成**。列入此表供后续规划参考，每项注明阻碍和验证方法。

| 序 | 优化项 | 潜在收益 | 阻碍 | 验证方法 |
|----|--------|---------|------|---------|
| O-1 | **bp FIR 拆分**: CNN 保留 1024tap, FxNLMS 改用 256tap (群延迟 32→8ms) | 宽带 NR +3-5dB | 需 Python 训练侧生成新 256tap bandpass FIR；子滤波器训练时用了 1024tap bp，换 bp 需重新验证 Ŝ 等效性 | A/B: 同噪声分别跑 1024/256tap bp 离线版，对比 NR_true |
| O-2 | **二值化 Wc 构造**: CNN 输出从 3 场景 softmax 改为 60 维 per-band 权重 + 阈值 ≥0.3 | 低置信度时避免弱激活子滤波器污染 Wc | 需重训 CNN（输出从 3 维改为 S×C=30 维）+ 重导 scene_defs.bin；完全改变 Wc 构造架构 | Python 端验证二值化 vs softmax 的 NR 差异，达标后移植 C |
| O-3 | **场景记忆衰减**: 超过 N 秒未使用的 scene_wc 逐步衰减回 CNN 预设 | 避免过期记忆污染 | 收益不确定：若噪声复现，保留已收敛 Wc 优于衰减 | 硬件 A/B: 同样噪声序列分别开关衰减,对比 NR |
| O-4 | **CNN 重叠窗口**: 1s 窗 0.5s 步长 → 场景响应从 1s 降到 0.5s | 场景切换响应更快 | CNN 在非重叠窗口上训练，改变窗口时序可能影响分类准确率；计算翻倍 | Python 端比较重叠/非重叠的分类准确率 |
| O-5 | **CNN 输入带宽扩展**: bp(20-1500Hz) → lowpass(2500Hz) | 更多高频信息辅助场景分类 | CNN 在 bp 输入上训练，换输入分布会破坏模型 | Python 端用 lowpass 重训 CNN，对比分类准确率 |

**当前结论**: R-11/R-12/R-47/R-48/R-52~R-57/R-33 已覆盖所有纯 C 优化。上述 5 项需跨项目协调（Python 训练 + C 移植），建议在下一轮训练迭代时一并评估。

**与 MIMO_GFANC (Python, 17.19dB) 的差距归因**: O-1 (bp 延迟) + O-2 (二值化 Wc) + 峰值归一化 (评测差异) + step_size 差异 (200×) + K=8 场景。其中仅 bp 延迟和 Wc 构造方式属于算法架构差异，其余为评测方法/参数选择差异。

### 7.4 建议修复路线

```
Phase-1 收尾:         R-58 (参数网格标定) → 全部完成 (38/55)
架构优化评估 (5):     O-1~O-5 待下一轮训练迭代评估 (均需 Python/硬件配合, 见 §7.3)
Python 训练侧 (2):    R-16-② (.bin格式头+export_bin.py) → R-13 (bp256tap新FIR系数)
硬件窗口期 (4):       R-16-① (Ŝ实测) → R-16-③ (Ŝ质量门禁) → R-50 (spk1重标) → R-51 (125Hz排查)
                       → R-13 A/B (bp256tap vs bp1024 硬件NR对比)
Phase-2 (6):          R-21/R-19 → R-29 (1×5×4) → R-24/R-27 → RPi 部署
Phase-3 (4):          芯片选型 → R-23/R-22/R-25/R-28 → 定点化仿真 (QCC 路线)
```
...
**仍开放（本报告对应项）**：F-B→R-16（拆分为 R-16-① Ŝ 实测 / R-16-② .bin 格式头 / R-16-③ 质量门禁）· P-1→R-14 ✅ · P-2→R-15 ✅ · TODO-1（centroid 数据已验证低风险，维持不改）· TODO-5 监控量→R-19 · B-1 反馈 IIR 环路（未实现，窗户场景影响有限，维持排队）· B-3 remove_notch 死代码（保留）· CR-7→R-33 ✅ (72164c8) · CR-17→R-31 ✅ · CRC-2/W-15 校准流水线（开放）· A1/A3/B1/B3/C2（升级建议，排队）· W-1~W-16 窗户声学/量产实验项（非代码，维持）· CR-19/W-10 场景覆盖度（训练侧，无法在本仓库验证）· R-50（spk1 标定待验证）· R-51（125Hz 声源待排查）
...
**硬件实测新发现（2026-07-28）**：R-36（calibrate 通道硬编码+设备过滤）· R-37+R-38（2026-07-29 改为 Ŝ 保持物理尺度、移除归一化）· R-39（默认 gain=10x 在反馈抵消缺失时触发啸叫）· R-40（README 快速开始缺校准步骤）
