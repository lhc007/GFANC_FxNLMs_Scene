...
| 序 | 项 | 级别 | 工作量 |
|----|----|------|--------|
| 8 | **R-5** WASAPI 变长回调 `fcount%3≠0` — 抽取相位跳动+输出尾部垃圾 | 严重 | 0.5 天 |
...
| 11 | **R-14** 48k→16k 最近邻抽取无抗混叠 — >8kHz 折叠入 ANC 通带 | 一般 | 0.2 天 |
| 12 | **R-15** 16k→48k ZOH 内插无抗镜像 — 镜像仅被 sinc 衰减 ~3.6dB | 一般 | 0.1 天 |
| 13 | **R-17** 啸叫检测三通道平均稀释单通道啸叫 ~5dB — 改取 max | 一般 | 0.2 天 |
| 14 | **R-18** 离线工具链: 24-bit WAV 不支持 + 重采样无抗混叠 + dB(Full) 瞬态 | 一般 | 0.3 天 |
| 15 | **R-20** CNN 双缓冲主线程超 1s 未消费静默丢帧 — 加 drop 计数日志 | 建议 | 0.1 天 |
| 16 | **R-32** 文档漂移 (README 8→6 场景、fade_len 16→1600、24-bit 声称等) | 建议 | 0.2 天 |
