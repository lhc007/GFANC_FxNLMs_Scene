...
#### R-37 Ŝ (secondary_path) 导出尺度非物理 — RMS=1.0 / peak=25.5 导致梯度失真 + NR 显示虚高 · **严重 · [Phase-1]**

- **位置**：`data/secondary_path.bin`（6144 floats, E=3 S=2 L=1024）+ export_bin.py:159（直接 `write_bin` 无归一化）
- **实测数据**：6 通道 peak 25.5/7.8/6.5/16.0/23.8/20.7，整体 RMS=1.0。正常声学脉冲响应应满足 peak≤1.0（直接路径衰减）、RMS≪1（1024 tap 窗口内能量远小于 1024）。
- **问题描述**：Ŝ 系数被整体放大 ~25-30×（疑似 Python 训练侧按 RMS=1 归一化而非 peak=1）。后果链：① Xd = Ŝ⊗ref 被放大 ~30× → 功率分母放大 ~900× → LMS 有效步长被压 ~900×（但被 step_size 历史调参吸收，表面"能跑"）；② antiEst = Wc⊗Xd 被放大 ~30× → NR 显示 40-74dB（真实值应 0-5dB）→ 日志完全不可信；③ 若对 Ŝ 做 peak→1.0 归一化（物理正确尺度），功率分母骤降 ~300× → 原 step_size 等效步长放大 ~300× → Wc 一秒内从 0.01 冲到 1.0+ 饱和 → 立即啸叫+freeze。
- **造成的影响**：NR 显示为噪声（用户误以为降噪极好）；任何 Ŝ 尺度修正必须同步重标定 step_size（R-38），否则直接破坏 LMS 稳定性。
- **修复方案**：两道工序缺一不可——① C 端加载后逐通道 peak→1.0 归一化（物理正确尺度）；② step_size 同步缩小 ~300×（1e-4→1e-6，默认值）。根治还需 Python 导出侧同步归一化（R-16 关联）。
- **验证方法**：antiEst 从 55-424 量级降至 0.1-1 范围（与 anti 同量级）；NR 从 40-74dB 降至 0-10dB；Wc 不再一秒冲顶。

#### R-38 Ŝ 归一化与 step_size 耦合 — 功率归一化的 Ŝ 尺度依赖性 · **一般 · [Phase-1]**

- **位置**：gfanc_types.h:54（`step_size` 默认值）、fxnlms_mimo.c:183-195（功率分母 ΣXd²）
- **问题描述**：功率归一化分母 `power[s] = (ε+ΣXd²)/(E·L)` 中 Xd = Ŝ⊗ref，故 power ∝ Ŝ_RMS²。step_size 的有效值 `μ_eff = step_size / power` 因此与 Ŝ 尺度强耦合。R-37 引入 peak→1.0 归一化使 Ŝ_RMS 从 ~1.0 降至 ~0.06 → power 降 ~300× → μ_eff 升 ~300×。原默认 step_size=1e-4 在归一化后等价于 μ_eff≈0.3（标准 NLMS 稳定上界 ~2），梯度过冲使 Wc 在 1 秒内从 ~0.01 增到 ~1.0。
- **造成的影响**：Ŝ 归一化（正确操作）与 step_size 默认值（历史调参产物）组合后 LMS 立即不稳定；用户按物理正确方式修正 Ŝ 反而"搞坏了"系统。
- **修复方案**：默认 step_size 1e-4→1e-6（与归一化后 Ŝ_RMS≈0.06 匹配），同时保留 `GFANC_STEP` 环境变量供现场调参。根治应在 fxnlms_mimo 初始化时根据实际 Ŝ_RMS 自适应标定 step_size（待 R-16 实测 Ŝ 后实施）。
- **验证方法**：归一化 Ŝ + step_size=1e-6 运行 10 秒 → Wc max 增长 < 0.02/s（不发散）；NR 缓慢增长（正常收敛速度）。
...
| 17 | **R-37** Ŝ (secondary_path) 导出尺度非物理 — peak→1.0 归一化 | 严重 | Phase-1 | 2026-07-28 |
| 18 | **R-38** Ŝ 归一化与 step_size 耦合 — 同步缩小 1e-4→1e-6 | 一般 | Phase-1 | 2026-07-28 |
...
| 27 | **R-47** x_hist 前向通路时间反转 — anti=Wc⊗x_hist 的 Wc 索引与梯度训练方向相反 (wc[0]对齐最旧而非最新样本) | 致命 | Phase-1 | 2026-07-28 |
| 28 | **R-48** NLMS 功率 floor 位置错误 — 有效 floor 被 E·L 除后缩小 3072×, 安静信号有效步长爆炸~1534 | 严重 | Phase-1 | 2026-07-28 |
| 29 | **R-49** antiEst/anti_rms 比值异常检测 — 比值>20× 时 NR 可能虚高, 新增诊断告警 | 一般 | Phase-1 | 2026-07-28 |
| 30 | **R-50** 反馈路径 spk1 校准极弱 (RMS≈2e-5) — R-47 修复后 anti 输出增大可能暴露反馈耦合 | 建议 | Phase-1 | 2026-07-28 |
| 31 | **R-51** 125Hz 窄带持续检测 — 房间驻波/环境声源在旧阈值边界间歇激活陷波 | 建议 | Phase-1 | 2026-07-28 |
| 32 | **P1** 输入电平 SPL 诊断 — ref RMS 过低/过高时自动建议 GFANC_MIC_GAIN 调节值 | 一般 | Phase-1 | 2026-07-28 |
| 33 | **P3** 啸叫检测阈值提高 — err 12→14dB, anti 10→12dB, 滤除 125Hz 环境窄带伪峰 | 一般 | Phase-1 | 2026-07-28 |
...
| 序 | 项 | 级别 | 工作量 |
|----|----|------|--------|
| 17 | **R-13** bp FIR 群延迟 32ms 扼守前馈通路 — 降阶 1024→256tap (8ms) 或 IIR | 严重 | 1 天 |
| 18 | **R-16** Ŝ 次级路径未实测校准 — `git revert 162d357` + 实测 E×S 条 Ŝ + bin 格式头 | 严重 | 1 天 |
...
| 24 | **R-27** 权重 .bin 无格式头/manifest — 版本混配无防线 | 一般 | 0.5 天 |
...
| 29 | **R-33** cnn_m5_free 空函数 + 权重永不释放 — 阻塞 OTA/模型热切换 | 建议 | 0.2 天 |
...
**硬件实测新发现（2026-07-28）**：R-36（calibrate 通道硬编码+设备过滤）· R-37（Ŝ 导出 RMS=1 非物理尺度+NR 虚高）· R-38（Ŝ 归一化与 step_size 耦合）· R-39（默认 gain=10x 在反馈抵消缺失时触发啸叫）· R-40（README 快速开始缺校准步骤）
