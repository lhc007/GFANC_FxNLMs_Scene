...
> 已完成 18 项 / 总计 35 项。按阶段分组，组内按严重度降序。
...
| 13 | **R-20** CNN 双缓冲丢帧检测 + 日志告警 | 建议 | Phase-1 | `fb64bdf` |

#### 🔴 Phase-1 待修复（纯软件，零硬件依赖）

| 序 | 项 | 级别 | 工作量 |
|----|----|------|--------|
| 14 | **R-11** anti_est 逐样本计算 6144 MAC/样本（~16% 回调预算）— NR 统计降采样优化 | 一般(Phase-1) / 严重(Phase-2/3) | 0.3 天 |
| 15 | **R-12** xd_roll_write/x_hist_push 全量 memmove 7168 拷贝/样本 — 改环形缓冲 | 一般(Phase-1) / 严重(Phase-2/3) | 0.5 天 |
| 16 | **R-18** 离线工具链: 24-bit WAV 不支持 + 重采样无抗混叠 + dB(Full) 瞬态 | 一般 | 0.3 天 |
