...
- **最终修复**（2026-07-29）：**彻底移除 Ŝ 归一化**，Ŝ 保持硬件实测的物理尺度。此时 Xd = Ŝ⊗ref_filt 在物理衰减下功率远小于 R-48 的 epsilon=1e-6 floor → power 由 epsilon 主导 → μ_eff = step_size/1e-6 ≈ 常数。step_size 恢复为固定 5e-7（μ_eff≈0.5，在 NLMS 稳定域 (0,2) 内安全），`GFANC_STEP` 可覆盖。同时移除 s_cal 字段，anti_est/NR 计算直接使用物理尺度值无需去归一化校准。
- **影响消除**：① antiEst/anti 比值回归合理范围（Ŝ 物理尺度下两者可比）；② step_size 不再与 Ŝ 尺度耦合；③ NR 公式恢复物理意义：`d = err - anti_est`（Ŝ 域扰动估计），与声级计测量值量级一致。
- **修复状态**：✅ **已修复** (2026-07-29) — 移除 main_realtime.c + main.c 的 Ŝ 归一化块，移除 s_cal 字段及统计去校准逻辑，step_size 固定 5e-7。
...
| 17 | **R-37+R-38** Ŝ 物理尺度 (移除归一化, s_cal, 固定 step_size=5e-7) | 严重 | Phase-1 | 2026-07-29 |
