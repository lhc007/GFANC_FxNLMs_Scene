...
| ✅ | F-B | 前馈 | 次级路径未实测 → 已测 | Ŝ C 版校准 + Python Farina 扫频 | `calibrate_secondary.c` + `export/measure_secondary.py` |
...
| ✅ | 冷启动防嗡嗡 | 安全 | 新场景 Wc 过强→瞬时 overshoot | 开机/切换可闻嗡嗡 | wc_cold_start 衰减 + cold_hold 2s 硬限幅±0.12 |
| ✅ | 场景记忆切换 | 功能 | 重复收敛浪费时间 | 切换后 NR 恢复慢 | 每场景保存/恢复 Wc |
| ✅ | 在线 Ŝ 辨识 | 算法 | Ŝ 静态→温湿度漂移 | NR 随运行时间下降 | `sec_online.c` NLMS μ=5e-6, 零探测噪声 |
| ✅ | 自适应 leak | 安全 | anti 偏高时 Wc 慢性漂移 | 嗡嗡尾音 | anti_rms 分级 2×/5×/10× leak |
| ✅ | AGC 输入 | 安全 | 突发大噪声→ADC 饱和 | 非线性失真触发发散 | 包络跟踪, 快攻慢放 |
| ✅ | Ŝ 自动标定 | 标定 | step_size 与 Ŝ 测量耦合 | 不同测量方法结果不可比 | Ŝ peak→1.0 归一化 + step∝1/s_rms² |
| ✅ | 测量工具链 | 工具 | 仅 C 白噪声法 (低 SNR) | Ŝ 不准 | Python Farina 扫频法 + 质量检验 |
