...
| F-I | 低 | 统计口径 (确认仍存在): `acc_anti` 在行212累积→safety_mute清零(行~234)→ramp缩放(行~238)。显示anti_rms高于实际输出, 仅影响监控显示, 不改变信号路径。位置: `main_realtime.c:212 vs :234,238`。 |
...
| ~~高~~ | ~~§6.1~~ | ~~fir_tick 取模~~ ✅ 已修复 (2026-07-22, 双段循环), xd 环形化留作独立优化 |
