...
3. **【低】CNN calloc**：`cnn_m5_forward` 每次推理 `calloc` 4×1MB（64×4000 float × 4 = 4MB），主线程 1Hz 调用无实时压力。建议一次性预分配静态缓冲，避免堆碎片化。Windows 默认堆对 1Hz 4MB 分配无实际影响，跨平台时注意嵌入式堆管理器差异。
...
5. **【中】无 Wc 发散防线**：仅有事后型 `safety_mute`（err_rms > ref_rms + ref_rms > 0.0001，每秒评估一次）。无 Wc 系数幅度监控、无变化率检测、无自动回退。Leak (1e-6, ~1.5%/秒) 提供有限正则化。F-A 修复 + ASIO 硬件降低了发散概率（不再有双重计入导致的错误梯度），但防线仍不完整。路线图 P1 "Wc 发散检测"已规划此缺口。
6. **【低】rt_ctx_t 栈分配**：~211KB（scene_wc 64KB + cnn_buf 128KB + wc_old/cur 16KB + 其他），Windows 1MB 栈足够。建议跨平台前改堆分配（嵌入式栈通常 < 256KB）。
...
| ~~中~~ | ~~F-D~~ | ~~leak 因子无效~~ ✅ 已修复 |
| 中 | S-1, S-2, F-C, F-E~F-G, P-1, P-2, §6.5 | 滞回/ramp/tanh/校准/混叠/Wc防线 |
| 低 | §6.3, §6.6, S-4, S-6, F-H, B-3, P-3, P-4 | calloc/栈/Blend/死代码 |
| ~~中~~ | ~~S-5, §6.4~~ | ~~MinMax~~ / ~~功率正则~~ **FALSE** |
| 🆕 已修复 | F-A, F-D, B-2, S-3, F-I, FIX-1~3,6 | 2026-07-22 系列修复 |
