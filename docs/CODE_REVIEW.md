...
| S-5 | 低 | **minmaxscaler 疑似不完整**：只除以 `(max-min)` 而不减 min（`scene_controller.c:40-50`）。若训练预处理是标准 MinMax（减 min 再除），此处不匹配。README 声称分类与 Python 一致，请以训练代码为准确认一次即可关闭此项。 |
...
| # | 严重程度 | 问题 |
|---|---|---|
| ~~F-A~~ | ~~**高**~~ | ~~**误差信号双重计入 + 扬声器信号被次级路径模型二次滤波**~~ **✅ 已修复 (2026-07-16)**。修复内容：新增 `fxnlms_tick_rt`/`fxnlms_forward_rt`（`fxnlms_mimo.c`），扬声器输出改为 `Wc ⊗ x`（新增 `x_hist`），自适应直接使用实测误差麦信号；`main_realtime.c` 回调改接线，NR 统计改为 `d̂=e−anti_est` vs `e`。数值验证 `tools/verify_fa.c`（完美模型 Ŝ=S 闭环仿真）：旧接线 SISO/MIMO 均为 **−14.6/−14.8 dB（放大噪声）**，新接线 **+11.4/+5.6 dB**。离线 `fxnlms_tick` 仿真语义保留不变。~~原描述：(1) 实时版把误差麦实测信号（物理上已含 `d + S·y`）作为 `disturbance` 传入，`fxnlms_tick` 再加一次 `anti_est`（`fxnlms_mimo.c:66-68`）→ 用于自适应的 `err ≈ d + 2·anti`，稳态最优解只能做到残差 = d/2，理论 NR 上限约 6dB。真实 FxLMS 应直接用实测误差麦信号做 e(n)。(2) 扬声器驱动信号 `anti_out[s] = Σ_e Σ_k Wc[s,k]·Xd[e,s,k]`（`fxnlms_mimo.c:70-74`）——物理输出应为 `Wc ⊗ x`（带通参考本身），这里输出的却是被 Ŝ 模型滤波并对 3 条误差路径求和后的信号，随后又经过**真实**次级路径 → 次级路径被作用两次、幅度 ×3。离线仿真中 `anti_out` 只写 WAV 无副作用，移植到实时后成为真 bug。~~ |
| F-B | ~~**高**~~ | ~~**sys_delay/因果对齐错误 + 次级路径从未实测**：`DSP_DELAY=16`（1ms @16k）用于右移次级路径模型（`main_realtime.c:346-356`），但 PortAudio 全双工的真实 DAC→ADC 往返通常 ≥10ms。Ŝ 与真实 S 的相位差在很多频点超过 ±90°，FxLMS 收敛条件被破坏。**且 `data/secondary_path.bin` 来自 Python 仿真工程，从未针对本硬件实测**；项目有反馈路径校准工具却没有次级路径校准工具——这是当前最根本的实机问题。~~ **🔧 已实施修复 (2026-07-16)**。新增 `calibrate_secondary.exe`（ZOH ×3 激励 16k 白噪声 — 与运行时输出路径一致, 逐扬声器 NLMS 辨识 6 条 Ŝ + 2 条反馈路径, 冲激响应内嵌全部 I/O 往返延迟和极性）；`main_realtime.c` 优先加载实测版（无 DSP_DELAY 偏移, 定标到 MIC_PRE_GAIN），缺失时回退仿真版 + 打印醒目警告；新增发散刹车（`anti_rms>0.90` → 回退 CNN 预设 + 作废污染场景记忆 + 重新冷启动），兼补 F-D 防线。**⚠️ 状态更新 (2026-07-16 深夜)**：v4 校准已在实机可靠测得环路延迟 ~78ms、流滑移 1484~1917ppm（双独立时钟 USB 设备 + Windows 共享音频栈的物理极限，白噪声 1 样本去相关导致辨识 ERLE≈0）。**软件侧修复已全部完成，此项剩余部分被硬件阻塞**——需单时钟 4in/2out 音频接口后重新校准方可关闭。完整分析与迁移清单见 [AUDIO_IO_MIGRATION.md](AUDIO_IO_MIGRATION.md)。 |
...
| F-D | **中** | **泄漏因子无效**：实际每样本衰减 `1 - step_size*leak = 1 - 1e-9`（`fxnlms_mimo.c:93-94`），一分钟仅衰减 ~0.1%，起不到抗漂移/抗发散作用。要么把 leak 提高 3-4 个数量级，要么按标准泄漏 LMS 写成 `wc *= (1 - μ·γ)` 并重新标定 γ。系数无任何幅度钳位，配合 F-A/F-B 的模型失配，**存在数值发散→满幅巨响的风险**（路线图 P1 "Wc 发散检测"已承认此缺口）。 |
...
| B-2 | **高** | **啸叫陷波器状态跨扬声器串用**：`howling_tick` 对 s=0、s=1 两路信号**复用同一组 biquad 状态** `x1/x2/y1/y2`（`howling_detect.c:190-197, 246-253`）——等效把两路不同信号交错喂进一个滤波器，两路输出都被污染。陷波激活时不但滤不干净，反而注入通道间串扰失真。状态数组必须扩展为 `[HW_MAX_NOTCHES][S]`。 |
...
4. **【中】除零/定义域**：`log10f`、cos 相似度、`construct_wc` 的 RMS 缩放均有 epsilon 防护 ✅；但 `fxnlms` 功率正则项被 `/(E*L)` 缩小到 1.6e-14（`fxnlms_mimo.c:76-84`），近静音时归一化增益极大，Wc 随底噪随机游走（leak 又无效，见 F-D）——正则项应在除以 `E*L` **之后**再加。
...
### 7.1 P0 — ~~修正实时 FxNLMS 信号结构（对应 F-A）~~ ✅ 已实施 (2026-07-16)
...
### 7.2 P0 — ~~实测次级路径与系统延迟（对应 F-B）~~ 🔧 已实施 (2026-07-16, 待实机校准验证)

新建 `calibrate_secondary.c`（复用 `calibrate_feedback.c` 的 NLMS 框架）：每只扬声器逐一播 ZOH 上采样的 16k 白噪声，用 3 只误差麦同时录音，NLMS 辨识 6 条 `Ŝ(e,s)`（含 I/O 往返延迟，**从而不再需要 `DSP_DELAY` 猜测值**），写入 `data/secondary_path_measured.bin`。运行时优先加载实测版本，缺失时回退仿真版并打印醒目警告。

### 7.3 P0 — 修复啸叫陷波状态串用（对应 B-2）
...
| leak 无效 + 无发散防线（F-D） | `wc *= (1-γ)`，γ≈1e-6/样本；每秒检查 `max|wc| > 5*stub_rms` → 冻结更新 + 淡回 CNN 预设（路线图方案可直接落地） |
...
| 等级 | 编号 | 一句话 |
|---|---|---|
| ~~高~~ | ~~F-A~~ | ~~实时误差合成错误 + 输出被 Ŝ 二次滤波，NR 理论上限 ~6dB~~ ✅ 已修复 (2026-07-16, `fxnlms_tick_rt` + 回调改接线, 验证见 `tools/verify_fa.c`) |
| ~~高~~ | ~~F-B~~ | ~~DSP_DELAY=1ms 远小于真实 I/O 延迟；次级路径从未实测~~ 🔧 calibrate_secondary.exe 已实现, 待实机校准 (2026-07-16) |
| 高 | B-2 | 啸叫陷波状态跨扬声器串用，激活即注入失真 |
| 高 | §6.2 | 主线程/回调对 Wc、fade_cnt 等无同步的数据竞争 |
| 高 | §6.1 | fir_tick 取模 + xd 全量搬移，回调 CPU 逼近 2ms 预算 |
| 中 | S-1, S-2, F-C~F-G, P-1, P-2, §6.4, §6.5 | 滞回失效、ramp 跳变、tanh 不连续、leak 无效、反馈校准失配、无抗混叠、正则量纲、无发散防线 |
| 低 | S-3~S-6, F-H, F-I, B-3, P-3, P-4 | 淡化尾帧、Blend 重标定 hack、统计口径、死代码等 |
