...
| 🔶 | F-B | 前馈 | 高 | 次级路径未实测校准 【需前置: ASIO校准】 | Ŝ 模型不准, LMS 收敛慢 | ASIO 硬件重新校准后 git revert |
...
| 🔶 | P-1 | 信号 | 中 | 48k→16k 抽取无抗混叠 【需前置: CPU预算】 | >8kHz 折叠, 硬件ADC+带通缓解 | FIR低通, CPU预算充裕时实施 |
| 🔶 | P-2 | 信号 | 中 | 16k→48k ZOH 无抗镜像 【需前置: CPU预算】 | 镜像频率, DAC+扬声器滚降缓解 | 零插值+低通, 与P-1一同实施 |
...
| 🔶 | §6.2 | 线程 | 高 | 主线程/回调对 fx.wc 等无同步 【需前置: 跨平台移植】 | x86 实测安全, ARM relaxed model 必崩 | 跨平台前修改 (影子缓冲+原子序号) |
...
> **图例**: ✅ 已修复 · 🔧 阻塞/待验证 · 🔶 需前置条件 · ❌ 未修复 · 🚫 伪错误(FALSE) · — 不适用/可忽略
